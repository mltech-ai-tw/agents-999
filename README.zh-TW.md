...
- **執行頁 (`/tools/[id]`)** — 每個代理人的動態表單、模型選擇器（只顯示你已設定金鑰的供應商）、
  即時串流輸出。兩種模式（見下方）：多輪**對話**串、或**對比**切換。
- **流程頁 (`/pipeline`)** — 把多個代理人串成一條循序執行的流程。
...
每個代理人都不只是一問一答——三種模式皆即時串流、且完全用你自己的金鑰執行：

- **💬 多輪對話** — 代理人回覆後，可繼續追問。每一輪都會把完整對話串重送給模型，
  因此它記得整段討論的脈絡。
- **⚖️ 跨模型對比** — 在任一代理人頁面開啟 **Compare** 切換，用相同輸入**最多並排執行
  3 組（供應商, 模型）**、同時串流。一眼看出 GPT、Claude、Gemini 對同一份需求的不同回答。
- **🔗 代理人流程** — 在 **`/pipeline`** 頁面排出一串步驟，每一步選一個代理人與一段指示，
  **上一步的輸出會成為下一步的輸入**——例如 *市場研究 → 定位 → 落地頁文案* 一鍵跑完。

> 任何模式下金鑰都只經過用過即丟的代理請求，且伺服器永遠重建每個代理人的 prompt——
> 對話歷史與流程上游內容只會被當作資料附加，不會覆蓋指令。
...
無帳號／OAuth · 無檔案／圖片輸入 · 無 UI 內建代理人建立 · 無使用分析 ·
無伺服器端儲存（對話與流程只存在開著的分頁中）。
